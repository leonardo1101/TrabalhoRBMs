...
testa.digitos <- function(modelo){
	...
	for (i in 1:length(padroes)) {
        ...
        retorno <- denoising.propagacao(modelo,entrada)
...
funcao.ruido <- function(x){
    return(x * 0.1)
}

#Arquitetura
arquitetura <-function(num.entrada,num.escondida,funcao.ativacao,der.funcao.ativacao,funcao.ruido){
	arq <- list()
...
denoising <- function(arq,dados,n,limiar){
	loss <- 10
	loss.anterior <- 0
	epocas <- 0
	...
	while (abs(loss - loss.anterior) > limiar){
		...
		for(i in 1:length(dados)){
			...
			resultado <- denoising.propagacao(arq,x)
...
denoising.propagacao <- function(arq,exemplo){
    # Entrada -> Ruido
    exemplo.ruido <- arq$funcao.ruido(exemplo)

	# Ruido -> Escondida
	v.entrada.escondida <- arq$escondida %*% as.numeric(c(exemplo.ruido,1))
...
arq <- arquitetura(length(padroes[[1]]),length(padroes[[1]])+60,funcao.ativacao,der.funcao.ativacao,0.1)
modelo <- denoising(arq,padroes,0.2,0.1)
